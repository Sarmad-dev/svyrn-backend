...
class SocketHandlers {
  ...
  async createUserSession(socket) {
    try {
      ...
      const existingSession = await UserSession.findOne({ userId: socket.userId });
  ...
  async handleSendMessage(socket, data) {
    try {
      ...
      conversation.participants.forEach(participant => {
        if (participant.user.toString() !== socket.userId && participant.isActive) {
  ...
  async sendNotificationToUser(userId, notification) {
    try {
      ...
      await savedNotification.populate('sender', 'firstName lastName profilePicture');
